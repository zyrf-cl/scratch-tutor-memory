...
- 短期上下文管理（**独立解耦功能**）：接收 agent 的运行对话，按 token / 轮次预算取回窗口（见「上下文管理」一节）
- 多学生隔离（按 `student_id` 分片）
- HTTP API + 一个跑得通的 demo 脚本（自带断言）

**不做（v1 不在范围内）**

- LLM 上下文的自动摘要压缩（短期上下文的缓冲 + 窗口化已作为**独立功能**提供，见「上下文管理」一节；基于 LLM 的溢出摘要仍未接入，只留好接口）
- 认证、限流、家长后台、班级聚合
...
- **上下文管理（独立功能，与上面解耦）**：`POST …/context/{session_id}/messages`、`GET …/context/{session_id}`、`GET …/context/{session_id}/stats`、`DELETE …/context/{session_id}` — 详见下方「上下文管理」一节
...
- 自带独立子包 `memory_module/context/`（`schemas` / `store` / `compaction` / `service` / `api`），独立的 `context_message` 表，与 `MemoryService` 不共享任何状态；只在 `build_app` 装配处挂一个 router。
- 默认与主库同文件（`MEMORY_MODULE_DB`），可用 `MEMORY_MODULE_CONTEXT_DB` 拆到单独文件。
- 窗口化策略由可插拔的 `Compactor` 决定，默认 `TruncatingCompactor`：`pinned` 消息（如 system prompt）永不裁掉，其余按「新 → 旧」填到 `max_tokens` / `max_turns` 预算用满为止；不传预算则全量返回。未来可换成基于 LLM 的摘要压缩器，接口不变。
...
| `MEMORY_MODULE_CONTEXT_DB` | 跟随 `MEMORY_MODULE_DB` | 上下文管理（独立功能）的存储文件；设了就把 `context_message` 表拆到单独的库 |
