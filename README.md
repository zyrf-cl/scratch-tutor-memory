...
| `AffectiveSignalDetector` | `StubAffectiveSignalDetector` | `MiMoAffectiveSignalDetector` | 其他 LLM / 多模态情感判别器 |

切换方法：

```bash
uv sync --extra st              # 装 sentence-transformers，embedder 自动切到真 BGE
export MEMORY_MODULE_USE_MIMO=1 # 启用 MiMo 摘要 + 迷思聚类 + 服务端情感判断（同时需要 MIMO_API_KEY）
...
- `POST /students/{sid}/agent-turns` — 提交一次 agent 诊断回合；服务端自动记录最近错误、反馈等级、薄弱概念，并自动生成情感事件
- `GET  /students/{sid}/agent-memory?session_id=...&task_id=...` — 读取 agent 决策快照（含 `recent_feedback_level` / `weak_concepts` / `affective_state`）
- `POST /students/{sid}/affective-events` — 可选，显式写入情感信号；通常只在外部已有专门情感判别器时使用
